...
today0 = date.today() - timedelta(days=2)
...
def giveme_real_title0(url):
    # url = 'https://www.donga.com/news/article/all/20230202/117699269/1'
    temp = requests.get(url)
    temp = BeautifulSoup(temp.content, 'html.parser')
    title0 = temp.find('h1', {'class':'title'}).text
    return title0
...
for i in range(s_last_ind, len(ind_list)):  #
    ...
    if '디지털스페셜' in str(line[1]) or '디지털콘텐츠' in str(line[1]):
        ...
        title0 = giveme_real_title0(line[3])
        print(title0)
